...
**Added**: performance_test_full_scale.py (commit d1ad3bc)

### Configuration
- **774 nodes** (full network)
- **2 age groups**, **25 heterogeneity bins**
- **117,648 total ODEs**
- **365 days** simulation
- **Metal backend** (GPU acceleration)
- **RK4 method** with tau_leap_dt=1.0
- Hybrid: stochastic threshold = 100
...
Setup time:      1.34s
Simulation time: 2.79s (kernel-reported)
```

**Efficiency**: ~86 simulation days per real-time second on M-series Mac GPU

### Simulation Results 📊

```
Peak infections: 66,989 at day 59
Attack rate:     89.1% (final recovered/total)
Nodes affected:  331/774 (42.8%)
Mass conservation: 1.29% error (acceptable with RK4+clamping)
```

### Diagnostic Figures Generated

1. **performance_test_diagnostics.pdf** (57KB) - 4-panel overview:
   - SEIR time series (S, E, I, R totals)
   - Prevalence heatmap (I/N by node over time)
   - Spatial spread (number of active nodes)
   - Epidemic wave timing (peak day by node)

2. **performance_test_heatmap.pdf** (94KB) - Detailed infection heatmap:
   - Log-scale infections by node over time
   - Timing annotations embedded in figure

**Note**: Figures are .gitignored but can be regenerated by running the script.

### Scalability Notes

For comparison with reduced test config (128 nodes, 2 age, 25 bins = 19,456 ODEs):
- 774 nodes config: **6× larger** problem (117K vs 19K ODEs)
- Full-scale timing: ~11.6ms/day
- Reduced config timing: ~7.7ms/day (from test_timestep_stability.py)
- **Overhead is sublinear**: 6× larger problem → only 1.5× slower

This suggests good GPU parallelization efficiency.
